...
export ARN64_BASE="/opt/Homebrew"
if [ "$1" == "buildserver" ] || [ "$2" == "buildserver" ]; then
    export ARN64_BASE="/usr/local"
fi
...
make clean
(SDL_CONFIG="${ARN64_BASE}/bin/sdl2-config" CC="clang -arch arm64 -mmacosx-version-min=10.7" make sdl-release -j$NCPU)
