...
cmake -DFLUIDSYNTH=OFF -DVORBIS=OFF -DBUILD_SHARED=ON -DBUILD_STATIC=ON -DOPENAL_LIBRARY=/usr/local/lib/libopenal.dylib -DOPENAL_INCLUDE_DIR=/usr/local/include/AL -DCMAKE_OSX_ARCHITECTURES="arm64;x86_64" -DCMAKE_POLICY_VERSION_MINIMUM=3.5 ../source/${SOURCE_FOLDER}
